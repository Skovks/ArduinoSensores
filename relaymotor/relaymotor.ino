...
///Datos de WiFi
const char* ssid = "bluetele7907";  // Aquí debes poner el nombre de tu red
const char* password = "01289898";  // Aquí debes poner la contraseña de tu red

//Datos del broker MQTT
const char* mqtt_server = "192.168.0.171"; // Si estas en una red local, coloca la IP asignada, en caso contrario, coloca la IP publica
...
// Variables
int flashLedPin = 15;  // Para indicar el estatus de conexión
...
void loop() {
  ...
  client.loop();
  }// fin del if (timeNow - timeLast > wait) // fin del void loop ()
...
void callback(char* topic, byte* message, unsigned int length) {
  ...
  if (String(topic) == "esp32/motor") {  // En caso de recibirse mensaje en el tema esp32/output
    if(messageTemp == "true"){
...
void reconnect() {
  ...
  while (!client.connected()) { // Pregunta si hay conexión
    ...
    if (client.connect("ESP32Client")) { //Pregunta por el resultado del intento de conexión
      ...
      client.subscribe("esp32/motor"); // Esta función realiza la suscripción al tema
